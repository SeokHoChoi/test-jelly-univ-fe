...
export default function PaymentResultPage() {
  ...
  useEffect(() => {
    try {
      ...
      const orderId =
        query.get("orderId") || sessionStorage.getItem("np_orderId") || "";
      const amountStr =
        query.get("amount") || sessionStorage.getItem("np_amount") || "0";
      const amount = Number(amountStr) || 0;

      // 결제한 플랜 타입 확인 (금액으로 판별)
      if (amount === 59000) {
        setPaidPlan("premium");
      } else {
        setPaidPlan("basic");
      ...
      (async () => {
        try {
          const response = await approvePayment(token, {
            tid,
            orderId,
            amount,
          });
          if (!response?.success)
            throw new Error(response?.message || "결제 승인 실패");
      ...
      <div className="p-4">
        <Card className="p-6 text-center max-w-md mx-auto">
          ...
          <div className="space-y-3">
            {!surveyCompleted ? (
              // 설문 미완료: 설문으로 자동 이동 유도
              <>
                <Button
                  onClick={() => router.push(`/survey?plan=${paidPlan}`)}
