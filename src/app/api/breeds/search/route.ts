...
const DOG_BREEDS = [
  ...
  '미니어처 슈나우저', '스탠다드 슈나우저', '자이언트 슈나우저', '화이트 슈나우저', '미니어처 화이트 슈나우저', '스탠다드 화이트 슈나우저', '자이언트 화이트 슈나우저',
  ...
  '아이리시 세터', '고든 세터', '잉글리시 세터', '아이리시 레드 앤 화이트 세터',
  '포인터', '잉글리시 포인터', '아이리시 포인터', '아이리시 레드 앤 화이트 포인터',
  '비즐라', '바이마라너', '체코슬로바키아 늑대개', '사를로스 늑대개', '타마스칸', '우타나간',
  '아메리칸 아키타', '아키타', '시바견'
];
...
export async function GET(request: NextRequest) {
  try {
    ...
    // 검색어와 일치하는 품종 필터링 (대소문자 구분 없음 + 유사 철자 지원)
    const filteredBreeds = DOG_BREEDS.filter(breed => {
      const breedLower = breed.toLowerCase();
      const queryLower = query.toLowerCase();

      // 정확한 매칭
      if (breedLower.includes(queryLower)) {
        return true;
      }

      // 유사 철자 매칭 (한글 자음/모음 변형)
      ...
      // 유사 철자 매칭 확인
      for (const [key, variations] of Object.entries(similarMatches)) {
        if (queryLower.includes(key)) {
          for (const variation of variations) {
            if (breedLower.includes(variation)) {
              return true;
            }
          }
        }
